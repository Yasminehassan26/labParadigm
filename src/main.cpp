    	...
    	while (!stack.empty() || currentFruit!=NULL ) {
			...
			if(currentFruit->getWeight()>weight){
                ...
                for(int it = 0; it<size; it++){
			        cout << "type:"<<weightList.front()->getType()<<"->"<<weightList.front()->getWeight()<< " ---> ";
...
void Tree:: magnifyByType(string type, int value){
    ...
    while(! fruits.empty()){
        Fruit* current= fruits.front();
        int newWeight = (current->getWeight()) + value;
        if(current->getType().find(type)!= string::npos){
            Fruit* deleted= deleteFruit(current->getWeight());
            Insert(NULL,root,deleted->getSpecific(),newWeight);
        }
        fruits.pop_front();
    }
...
//cout<<" min : type is :"<<min->getWeight()<< "  and  " <<min->getWeight()<<"\n";
Fruit* max=b.findHeaviest();

cout<<" max is "<<max->getType()<<"\n";
b.filterByWeight(20);
